...
if ($period == 'ELECTION DAY') {
	$section1 = base64_encode($section);
	$m_encode1 = base64_encode($m_encode);
$sql3 = mysql_query("select * from result where sch_session = '$section1' and matric_no = '$m_encode1'");
...
header("location: voters/");
	...
	//$password = base64_encode($password);
$sql = mysql_query("select * from create_login where username = '$username' and password = '$password' and t_section = 'Electoral Representative'");
...
$sql3 = mysql_query("select * from create_login where username = '$username' and password = '$password' and t_section = 'Election Observer'");
if(mysql_num_rows($sql3)) {
$_SESSION['username'] = $username;
$_SESSION['password'] = $password;
...
<input type="submit" name="submit" id="submit" value="Login to Vote" class="btn btn-primary btn-md btn-block waves-effect text-center m-b-20">                                    </div>
                                <a href="f_password.php">forgot password</a></div>
                                <hr/>
                                <div class="row">
